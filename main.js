...
// subcategoría (ej. solo "Clósets" dentro de "Recámara") aún no está
// implementado en el catálogo público — queda pendiente como tarea aparte.
window.irACategoria = function (catId) {
  document.getElementById("navDrawer")?.classList.remove("open");
  document.getElementById("navOverlay")?.classList.remove("open");
  document.body.style.overflow = "";
  const btn = [...document.querySelectorAll(".filter-tab")].find(b => b.getAttribute("onclick")?.includes(`'${catId}'`));
  if (btn) { btn.click(); }
