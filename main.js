...
window.MG = {
  ...
  banners:   [],
  likesCount: {},    // { productoId: cantidad }
  db, auth, WA_NUMBER
};
...
// ============================================
//  NAVEGACIÓN / DRAWER (acordeón de categorías)
// ============================================
function renderNav() {
...
// ============================================
//  TARJETA DE PRODUCTO (compartida por todas las páginas)
// ============================================
window.nombreCategoria = function (p) {
  const cat = (MG.categorias || []).find(c => c.id === p.categoriaId);
  if (!cat) return p.categoria || ""; // compatibilidad con productos viejos sin categoriaId
  const sub = p.subcategoriaId ? cat.subcategorias.find(s => s.id === p.subcategoriaId) : null;
  return sub ? `${cat.nombre} · ${sub.nombre}` : cat.nombre;
};

window.buildCard = function (p) {
  const favs  = MG.favoritos || [];
  ...
  const galeria = imgs.length > 1
    ? `<div class="card-thumbs">${imgs.map((url, i) =>
        `<button class="card-thumb-dot ${i === 0 ? "active" : ""}" onclick="cambiarImagenCard(this, '${p.id}', ${i})" aria-label="Foto ${i+1}"></button>`
      ).join("")}</div>`
    : "";
  return `
    <div class="card" id="prod-${p.id}">
      <div class="image">
        <img id="img-${p.id}" src="${imgs[0] || ""}" alt="${p.nombre}" onerror="this.style.opacity='0.2'" data-imgs='${JSON.stringify(imgs)}'>
        <button class="card-fav-btn ${saved ? "saved" : ""}" data-id="${p.id}" onclick="toggleFavorito('${p.id}')" title="Guardar en favoritos">
      ...
      <div class="card-info">
        <span class="categoria-tag">${window.nombreCategoria(p)}</span>
        <span class="title">${p.nombre}</span>
        ${p.descripcion ? `<span class="descripcion">${p.descripcion}</span>` : ""}
        <span class="price">$${Number(p.precio).toLocaleString("es-MX")} MXN</span>
      </div>
      <div class="card-actions">
        <button class="btn-comprar" onclick="abrirWAPedido('${p.nombre}', ${p.precio}, '${p.descripcion || ""}')">
        ...
        <button class="btn-contactar" onclick="abrirWAConsulta('${p.nombre}')">
...
window.cambiarImagenCard = function (btn, prodId, idx) {
  const img = document.getElementById(`img-${prodId}`);
  if (!img) return;
  const imgs = JSON.parse(img.dataset.imgs || "[]");
  if (!imgs[idx]) return;
  img.src = imgs[idx];
  btn.parentElement.querySelectorAll(".card-thumb-dot").forEach(d => d.classList.remove("active"));
...
window.toggleFavorito = async function (productoId) {
  ...
  // Contador público de "me gusta" (usado para "El más querido"). Va en
  // colección aparte, de solo-conteo, para no exponer quién le dio like
  // a qué (favoritos/{uid} sigue siendo privado, solo el dueño lo lee).
  try {
    await runTransaction(db, async (tx) => {
      const ref = doc(db, "producto_likes", productoId);
      const snap = await tx.get(ref);
      const actual = snap.exists() ? (snap.data().count || 0) : 0;
      const nuevo = Math.max(0, actual + (agregando ? 1 : -1));
      tx.set(ref, { count: nuevo }, { merge: true });
    });
    MG.likesCount[productoId] = Math.max(0, (MG.likesCount[productoId] || 0) + (agregando ? 1 : -1));
    renderTopFavoritos();
  } catch (e) {
    console.warn("No se pudo actualizar el contador de likes:", e);
  }
...
// ── Top 3 productos más queridos (por conteo público de likes) ──
async function cargarLikesCount() {
  const snap = await getDocs(collection(db, "producto_likes"));
  const counts = {};
  snap.forEach(d => { counts[d.id] = d.data().count || 0; });
  MG.likesCount = counts;
}
window.cargarLikesCount = cargarLikesCount;

function getTop3Favoritos() {
  return MG.productos
    .map(p => ({ producto: p, count: MG.likesCount[p.id] || 0 }))
    .filter(x => x.count > 0)
    .sort((a, b) => b.count - a.count)
    .slice(0, 3);
}

function renderTopFavoritos() {
  const cont = document.getElementById("topFavoritosGrid");
  const section = document.getElementById("topFavoritosSection");
  if (!cont || !section) return;

  const top3 = getTop3Favoritos();
  if (top3.length === 0) {
  ...
  cont.innerHTML = top3.map((item, i) => `
    <div class="top-fav-card">
      <span class="top-fav-medalla">${medallas[i]}</span>
      ${typeof window.buildCard === "function" ? window.buildCard(item.producto) : ""}
    </div>
  `).join("");
}
window.renderTopFavoritos = renderTopFavoritos;
...
// Botón "Contactar": consulta suave, sin comprometerse a comprar,
// distinto del mensaje de "Pedir por WhatsApp" (que ya implica intención de compra).
window.abrirWAConsulta = function (producto) {
  const text = encodeURIComponent(
    `Hola, tengo una pregunta sobre este mueble: ${producto}. ¿Me pueden dar más información?`
  );
  window.open(`https://wa.me/${WA_NUMBER}?text=${text}`, "_blank");
};
  ...
  list.innerHTML = MG.productos.map(p => {
    ...
    <div class="admin-prod-row" id="row-${p.id}">
      ...
      <div class="admin-prod-fields">
        ...
        <p class="img-hint">📐 1448×1448px, formato .webp</p>
...
window.volverAProductosDesdeAgregar = function () {
  mostrarTab("tabProductos");
  document.getElementById("btnVolverProductos").style.display = "none";
...
window.agregarProducto = async function () {
  ...
  try {
    ...
    msg.className = "admin-msg admin-msg-grande";
    msg.style.color = "#27ae60";
    msg.textContent = `✅ ¡Producto agregado con ${imagenes.length} foto(s)!`;

    // Limpiar TODO el formulario, no solo la foto, para el siguiente producto
    document.getElementById("nuevoNombre").value = "";
    document.getElementById("nuevoPrecio").value = "";
    document.getElementById("nuevaCategoria").selectedIndex = 0;
    actualizarSelectSubcategoria(document.getElementById("nuevaSubcategoria"));
    document.getElementById("nuevaImagen").value = "";
    document.getElementById("nuevaDesc").value = "";
    if (document.getElementById("nuevoDestacado")) document.getElementById("nuevoDestacado").checked = false;

    document.getElementById("btnVolverProductos").style.display = "block";
    renderAdminProductos();
